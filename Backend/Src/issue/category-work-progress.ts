/** How assigned staff are expected to work the ticket (UI + sub-status validation). */
...
export function allowedWorkProgressValues(category: string): string[] {
  return getCategoryTechnicianWorkflow(category).progressOptions.map((o) => o.value);
}

export function isValidWorkProgressForCategory(
  category: string,
  value: string,
): boolean {
  return allowedWorkProgressValues(category).includes(value);
}

/** Optional tighter resolution hint for common billing subcategories. */
