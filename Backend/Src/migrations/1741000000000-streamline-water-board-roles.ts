...
/**
 * Streamlined water-board roles (2026): six roles, single `department_officer` + User.department
 * for all unit queues (billing, metering, water quality, etc.).
 */
export class StreamlineWaterBoardRoles1741000000000 implements MigrationInterface {
  name = 'StreamlineWaterBoardRoles1741000000000';

  public async up(queryRunner: QueryRunner): Promise<void> {
    // Add new role labels before UPDATE (MySQL rejects values not in the current ENUM).
  ...
  public async down(): Promise<void> {
    throw new Error(
      'StreamlineWaterBoardRoles1741000000000 cannot be reverted safely — restore database from backup if needed.',
    );
  }
}
