...
exports.ProductionHardening1730128000000 = void 0;
class ProductionHardening1730128000000 {
    name = 'ProductionHardening1730128000000';
    async up(queryRunner) {
        const hasColumn = async (table, column) => {
            const rows = (await queryRunner.query(`SELECT COUNT(*) as c FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = ? AND COLUMN_NAME = ?`, [table, column]));
            return Number(rows[0]?.c) > 0;
        };
        if (!(await hasColumn('users', 'failed_login_attempts'))) {
            await queryRunner.query('ALTER TABLE `users` ADD `failed_login_attempts` int NOT NULL DEFAULT 0');
        }
        if (!(await hasColumn('users', 'lockout_until'))) {
            await queryRunner.query('ALTER TABLE `users` ADD `lockout_until` datetime NULL');
        }
        if (!(await hasColumn('issues', 'reporter_email'))) {
            await queryRunner.query('ALTER TABLE `issues` ADD `reporter_email` varchar(190) NULL');
        }
    ...
    async down(queryRunner) {
        ...
        await queryRunner.query('ALTER TABLE `issues` DROP COLUMN `reporter_email`');
...
exports.ProductionHardening1730128000000 = ProductionHardening1730128000000;
//# sourceMappingURL=1730128000000-production-hardening.js.map